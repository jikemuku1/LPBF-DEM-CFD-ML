...
def create_comprehensive_features(df):
    ...
    df['Power_per_Area'] = power / (np.pi * radius ** 2)  # 修复：添加除法符号
    df['Power_per_Radius'] = power / radius  # 修复：变量名大小写一致
    ...
    df['Fusion_Parameter'] = (power * radius) / velocity  # 修复：正确的公式
...
# [5] Define Bayesian Ridge parameters
print("\n[5/9] Setting up Bayesian Ridge with enhanced features...")

param_grid = {
    "n_iter": [100, 300, 500, 1000],
    "alpha_1": [1e-6, 1e-7, 1e-8, 1e-9],
    "alpha_2": [1e-6, 1e-7, 1e-8, 1e-9],
    "lambda_1": [1e-6, 1e-7, 1e-8, 1e-9],
    "lambda_2": [1e-6, 1e-7, 1e-8, 1e-9],
...
# [6] Training function
def train_and_evaluate_br(X, y, param_grid, kf, target_name):
    """Train and evaluate Bayesian Ridge with honest cross-validation"""
    print(f"Training Bayesian Ridge for {target_name}...")
    ...
    y_pred = cross_val_predict(best_br, X, y, cv=kf, n_jobs=-1)
    ...
    print(f"  ✓ Bayesian Ridge - {target_name}: R² = {r2:.4f}, MSE = {mse:.4e}, Time = {training_time:.2f}s")
    print(f"  Best parameters: {grid_search.best_params_}")

    return best_br, y_pred, mse, r2, mape, training_time


# [7] Train models
print("\n[6/9] Training Bayesian Ridge models with engineered features...")

best_br_width, y_pred_width, mse_width, r2_width, mape_width, time_width = train_and_evaluate_br(
    X_normalized, y_width_normalized, param_grid, kf, "Width"
)

best_br_depth, y_pred_depth, mse_depth, r2_depth, mape_depth, time_depth = train_and_evaluate_br(
    X_normalized, y_depth_normalized, param_grid, kf, "Depth"
)

best_br_porosity, y_pred_porosity, mse_porosity, r2_porosity, mape_porosity, time_porosity = train_and_evaluate_br(
    X_normalized, y_porosity_normalized, param_grid, kf, "Porosity"
)

# [8] Results summary
print("\n[7/9] Bayesian Ridge Results Summary:")
...
importance_width = analyze_feature_importance(X_normalized, y_width_normalized, feature_names, best_br_width, "Width")
importance_depth = analyze_feature_importance(X_normalized, y_depth_normalized, feature_names, best_br_depth, "Depth")
importance_porosity = analyze_feature_importance(X_normalized, y_porosity_normalized, feature_names, best_br_porosity,
                                                 "Porosity")
...
def plot_combined_results_normalized(y_true_width, y_pred_width, y_true_depth, y_pred_depth,
    ...
    plt.title('Bayesian Ridge Cross-Validation Results (Normalized)', fontsize=20, fontweight='bold')
    ...
    plt.savefig("Fig13f_BayesianRidge_CrossVal_Results.png", dpi=300, bbox_inches="tight")
plt.close()
...
# [11] Bayesian Ridge specific analysis
print("\n[10/10] Bayesian Ridge Specific Analysis:")


def analyze_bayesian_features(model, feature_names, target_name):
    """Analyze Bayesian Ridge specific features"""
    ...
    print(f"\nTop 10 coefficients for {target_name}:")
    for i, row in coef_df.head(10).iterrows():
        sign = '+' if row['Coefficient'] >= 0 else '-'
        print(f"  {row['Feature']}: {sign}{abs(row['Coefficient']):.4f}")

    print(f"  Intercept: {intercept:.4f}")
    print(f"  Precision (alpha): {alpha:.4e}")
    print(f"  Regularization (lambda): {lambda_:.4e}")

    return coef_df


print("Bayesian Ridge coefficient analysis:")
coef_width = analyze_bayesian_features(best_br_width, feature_names, "Width")
coef_depth = analyze_bayesian_features(best_br_depth, feature_names, "Depth")
coef_porosity = analyze_bayesian_features(best_br_porosity, feature_names, "Porosity")

# [12] Final summary with R² filtering
print("\n[11/11] Final Bayesian Ridge Model Performance with R² Filtering:")

# Count how many targets meet the R² criteria
valid_targets = []
if 0.8 <= r2_width <= 1.2:
    valid_targets.append(f"Width (R²={r2_width:.4f})")
if 0.8 <= r2_depth <= 1.15:
...
print(f"Depth: {r2_depth:.4f} {'✓' if 0.8 <= r2_depth <= 1.15 else '✗'}")
