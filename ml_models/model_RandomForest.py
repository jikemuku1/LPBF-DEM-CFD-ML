...
# [5] Define Random Forest parameters
print("\n[5/9] Setting up Random Forest with enhanced features...")
...
# [6] Training function
def train_and_evaluate_rf(X, y, param_grid, kf, target_name):
    """Train and evaluate Random Forest with honest cross-validation"""
    ...
    print(f"  ✓ Random Forest - {target_name}: R² = {r2:.4f}, MSE = {mse:.4e}, Time = {training_time:.2f}s")
    print(f"  Best parameters: {grid_search.best_params_}")

    return best_rf, y_pred, mse, r2, mape, training_time


# [7] Train models
print("\n[6/9] Training Random Forest models with engineered features...")

best_rf_width, y_pred_width, mse_width, r2_width, mape_width, time_width = train_and_evaluate_rf(
    X_normalized, y_width_normalized, param_grid, kf, "Width"
)

best_rf_depth, y_pred_depth, mse_depth, r2_depth, mape_depth, time_depth = train_and_evaluate_rf(
    X_normalized, y_depth_normalized, param_grid, kf, "Depth"
)

best_rf_porosity, y_pred_porosity, mse_porosity, r2_porosity, mape_porosity, time_porosity = train_and_evaluate_rf(
    X_normalized, y_porosity_normalized, param_grid, kf, "Porosity"
)

# [8] Results summary
print("\n[7/9] Random Forest Results Summary:")
...
def plot_combined_results_normalized(y_true_width, y_pred_width, y_true_depth, y_pred_depth,
    ...
    plt.title('Random Forest Cross-Validation Results (Normalized)', fontsize=20, fontweight='bold')
    ...
    plt.savefig("Fig13d_RandomForest_CrossVal_Results.png", dpi=300, bbox_inches="tight")
plt.close()
